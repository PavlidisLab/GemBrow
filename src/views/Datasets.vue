...
<template>
    ...
    >
        <template slot="settingsForm">
            ...
            <v-slider :label="score_q_min.toFixed(1).toString()" :disabled="!score_q_min_on" v-model="score_q_min"
                      thumb-label step="0.1" ticks min="-1" max="1"></v-slider>
            <v-divider/>
            <v-switch v-model="score_s_min_on" label="Min. suitability:"/>
            <v-slider :label="score_s_min.toFixed(1).toString()" :disabled="!score_s_min_on" v-model="score_s_min"
            ...
            <v-layout row wrap>
                ...
                <v-checkbox v-model="publication" :disabled="!publication_on"
                            :label="'Only with'+(+!publication ? 'out':'')+' publication'"/>
            </v-layout>
        </template>
    </DataPage>
</template>
            ...
            return {
                ...
                cols: [
                    ...
                    },
                    {
                        ...
                        show: true,
                        rowTip(props){
                            return props.item.needsAttention ? "Curation not finished" : "Curated";
                        },
                        icon(props) {
                    ...
                    },
                    {
                        ...
                        iconStyle(props) {
                            return "background-color: "+viewUtils.methods.scoreToColor(
                                props.item.geeq.publicQualityScore
                            );
                        },
                        iconClass(){
